...
class MPLUG:
    def __init__(self, ckpt='damo/mplug_visual-question-answering_coco_large_en'):
        self.pipeline_vqa = pipeline(Tasks.visual_question_answering, model=ckpt, model_revision="v1.0.0")
...
class BLIP2:
    ...
    def vqa(self, image, question, choices = []):
        ...
        if len(choices) == 0:
            ...
            answer = self.model.generate({"image": image, "prompt": f"Question: {question} Options: {', '.join(choices)} Answer:"})
        return answer[0]
